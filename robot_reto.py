...
robot.avanzar(-300) #empujar la pala a su lugar
...
robot.avanzar(-110) #retroceso para meter un poco mas el cemento en su lugar


#seccion 6: Camino a los cementos blancos-----------------------------------------------------------------
robot.reset_imu_y_base() #reseteo

robot.movimiento_giroscopio(850, 19) #movimeinto hacia delante despues de dejar el cemento
robot.reset_imu_y_base() #reseteo
robot.girar_giroscopio(500, -90) #giro en direccion a los cementos
robot.esperar(100) #pausa
robot.reset_imu_y_base() #reseteo
robot.seguir_linea_con_giroscopio(distancia_cm=25, velocidad=1000, ref=30)
robot.reset_imu_y_base() #reseteo
robot.girar_giroscopio(450,-180) #giro para poner la celda sobre los cementos
robot.avanzar(-95) #avance de seguridad
robot.mover_torque(400, 80)#agarrar los cementos blancos
...
robot.girar_giroscopio(500, -52) #giro para quedar recto sobre la linea
robot.esperar(100)
robot.reset_imu_y_base()
robot.seguir_linea_con_giroscopio(distancia_cm=17, velocidad=1000, ref=30) #seguidor para ajustarse
robot.girar_giroscopio(450, 180) #Giro de 180 para poner la celda viendo a la matriz
robot.esperar(100)
robot.giro_derecha(500, -35) # giro para entrar en el espacio indicado 27.5
...
robot.reset_imu_y_base()
robot.movimiento_giroscopio(850, 26) #retroceso de salida
robot.reset_imu_y_base()
robot.giro_derecha(500, 30) #Giro para acomodarse
robot.reset_imu_y_base() 
...
robot.reset_imu_y_base()
robot.movimiento_giroscopio(600, 12)  #movimiento para avanzar
robot.reset_imu_y_base()
robot.giro_derecha(500, 85) #segundo giro para ir al amarillo
robot.reset_imu_y_base()
robot.movimiento_giroscopio(350, -33) #movimiento para ir hacia los cementos amarillos
